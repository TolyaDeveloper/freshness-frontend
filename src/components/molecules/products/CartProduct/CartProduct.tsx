...
const CartProduct = ({ className, product }: CartProductProps) => {
  ...
  const [productAmount, setProductAmount] = useState<number>(quantity)
  const [selectedType, setSelectedType] = useState(variant)

  const onRemoveFromCart = async (_id: string) => {
    if (isAuthenticated) {
      const { data: updated } = await userService.removeFromCart(
        product.productId
      )
  ...
  return (
    ...
    //   <Link href={`${PAGES.products}/${productId}`} prefetch={false}>
